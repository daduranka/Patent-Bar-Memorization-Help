...
#hint = ''
sections_list = sections_to_memorize.keys()

mode = 1
if mode == 1:
    for section in sections_list:
        hint = ''
        letterOne = random.randrange(len(section))
        letterTwo = random.randrange(len(section))
        #Cycle through letters and substitute the letters with dashes if necessary
        option = section.split() 
        for letter in section:
            if letter == section[letterOne] or letter == section[letterTwo]:
                hint += f"_"
            else:
                hint += f"{letter}"
        guess = input(f'Your hint is: {hint}; What is the complete answer? ')
        if guess.lower() != section.lower():
            print(f'Sorry "{guess}" is not the correct answer, the correct answer is "{section}"')
        else:
    ...
    for key in sections_to_memorize:
        ...
        for item in sections_to_memorize[key]:
            ...
            if len(option) == 0:
                ...
                if guess != item:
                    print(f'Sorry that is not the correct answer, the correct answer is "{item}"')
                else:
